...
$(document).ready(function() {
    ...
    inputSearch.addEventListener("input", function(event) {
        autocompleteGwz(this);
    });
    if (typeof term !== 'undefined') {
        inputSearch.focus();
        inputSearch.value = term;
        autocompleteGwz(inputSearch);
    } else {
        inputSearch.value = '';
    }
    ...
    $('#btn_connexion_responsive').on('click', function() {
        ...
        $.ajax({
            ...
            if (data.erreur) {
                ...
                } else if (redirection == 'return') {
                    window.location.href = 'my-account/return';
                } else if (redirection == 'return_sav') {
                    window.location.href = 'my-account/sav';
                } else if (redirection == 'compte') {
    ...
    $('.scrollManufacturers').click(function() {
        var id = $(this).attr('rel');
        $("body,html").animate({
            scrollTop: $('#alpha_' + id).offset().top
        }, 800);
    });
    $('.topManufacturers').click(function() {
        $("body,html").animate({
    ...
    $('.toggle').on('click', function() {
        $('.custom-modal-menu').stop().addClass('active');
    });
    $('.close-animated-menu').on('click', function() {
        $('.custom-modal-menu').stop().removeClass('active');
    });
