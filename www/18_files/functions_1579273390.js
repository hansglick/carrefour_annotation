...
var modal_forgot_password = function(id) {
    ...
    $.getJSON("./ajax/forgot_password.php", {
        "action": "process",
        "email_address": my_login
    }, function(data) {
        if (!data.erreur) {
...
// Scroll infini moteur de recherche
var launchElasticAutoComplete = function() {
    var viewportPaginationAutoComplete = document.getElementById('viewport_pagination');
    var isOut = isOutOfViewportAutoComplete(viewportPaginationAutoComplete);
    if (isOut.any) {
        // console.log('Not in the viewport! =(');
    } else {
        document.getElementById('search-ul-id-1').removeEventListener('scroll', launchElasticAutoComplete);
        //console.log('In the viewport! =)');
        if (parseInt($('#page').val()) <= parseInt($('#total_page').val())) {
            paginationScrollAutocomplete();
            timer = setTimeout(function() {
                document.getElementById('search-ul-id-1').addEventListener('scroll', launchElasticAutoComplete);
            }, 300);
        }
    }
}
 ...
 */
function isOutOfViewportAutoComplete(elem) {
    // console.log('isOutOfViewportAutoComplete');
...
/*            viewPortDiv = document.createElement("div");
            viewPortDiv.setAttribute('id', 'viewport_pagination');
            document.getElementById('results_search').appendChild(viewPortDiv);*/
        }
    ...
    input.addEventListener("blur", function(event) {
        ...
        // dataLayer push for Google Tag Manger and Google Analytics site search
        dataLayerAutocompleteSearch(searchedTerms);
        // old ga push
        if (window.location.pathname.indexOf('lemarche') !== -1) {
            ga('send', 'pageview', "lemarche/autocomplete_search.php?keywords=" + searchedTerms);
        } else {
...
var display_connexion = function() {
    if ($(window).width() < 992) {
        ...
        window.location.href = 'responsive.php?action=connexion&page=compte';
    } else {
